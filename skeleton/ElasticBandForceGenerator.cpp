...
void ElasticBandForceGenerator::updateForce(RigidSolid* s, double t)
{
	if (other) {
		//vector que conecta las particulas
		Vector3 relative_pos_vector = other->getPos() - s->getPosition();

		float length = relative_pos_vector.normalize();	//distancia entre los dos solidos
